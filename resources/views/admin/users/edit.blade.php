...
<style>
    ...
    .btn-submit { display: inline-flex; align-items: center; gap: 8px; padding: 12px 28px; background: linear-gradient(135deg,#8b5cf6,#7c3aed); border: none; border-radius: 10px; color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; transition: all 0.2s; }
    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-control" placeholder="John Doe" value="{{ old('name', $user->name) }}" required>
                @error('name')<div class="form-error">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label for="email">Email Address <span style="color:#f87171;">*</span></label>
                <input type="email" id="email" name="email" class="form-control" placeholder="john@example.com" value="{{ old('email', $user->email) }}" required>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="position" name="position" class="form-control" placeholder="e.g. Software Developer" value="{{ old('position', $user->position) }}">
            ...
            <div class="form-group">
                ...
                <select id="role" name="role" class="form-control" required>
                    <option value="user" {{ old('role', $user->hasRole('super-admin') ? 'super-admin' : 'user') == 'user' ? 'selected' : '' }}>User</option>
                    <option value="super-admin" {{ old('role', $user->hasRole('super-admin') ? 'super-admin' : 'user') == 'super-admin' ? 'selected' : '' }}>Super Admin</option>
                </select>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" id="sort_order" name="sort_order" class="form-control" value="{{ old('sort_order', $user->sort_order) }}">
            ...
            <div class="form-group">
                ...
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
                    <img src="{{ $user->profile_photo_url }}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;" alt="">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <div class="toggle-row">
                    <label class="toggle-switch">
                        <input type="checkbox" name="is_visible" {{ old('is_visible', $user->is_visible) ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <div class="toggle-row">
                    <label class="toggle-switch">
                        <input type="checkbox" name="can_login" {{ old('can_login', $user->can_login) ? 'checked' : '' }}>
                        <span class="toggle-slider"></span>
                    </label>

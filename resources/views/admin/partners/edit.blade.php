...
<style>
    ...
    .form-card { background: #fff; border: 1px solid var(--border); border-radius: 16px; padding: 32px; max-width: 680px; }
    ...
    .logo-preview img { width: 100px; height: 60px; object-fit: contain; background: #fff; border-radius: 8px; padding: 5px; border: 1px solid var(--border); }
    ...
    <form method="POST" action="{{ route('admin.partners.update', $partner) }}" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-control" placeholder="e.g. BARAKA HOSPITAL" value="{{ old('name', $partner->name) }}" required>
                @error('name')<div class="form-error">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label for="activity">Activity / Area <span style="color:#f87171;">*</span></label>
                <input type="text" id="activity" name="activity" class="form-control" placeholder="e.g. ICT SOLUTIONS" value="{{ old('activity', $partner->activity) }}" required>
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" class="form-control" rows="4" placeholder="Brief description of this partner...">{{ old('description', $partner->description) }}</textarea>
        ...
        <div class="form-group">
            ...
            <div class="toggle-row">
                <label class="toggle-switch">
                    <input type="checkbox" name="is_active" {{ old('is_active', $partner->is_active) ? 'checked' : '' }}>

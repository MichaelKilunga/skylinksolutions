...
<style>
    ...
    .avatar { width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 700; color: #fff; flex-shrink: 0; background: linear-gradient(135deg,#10b981,#06b6d4); }
    .skills-tag { display: inline-block; background: rgba(139,92,246,0.15); color: #a78bfa; font-size: 11px; font-weight: 600; padding: 3px 8px; border-radius: 6px; margin: 2px; }
            ...
            <tr>
                ...
                <td>
                    <div class="name-cell">
                        ...
                        <span style="font-weight:600;color:#e2e8f0;">{{ $vol->name }}</span>
                    </div>
                </td>
                <td>
                    <div>{{ $vol->email }}</div>
                ...
                <td>
                    <div style="display:flex; gap:8px;">
                        <a href="{{ route('admin.volunteers.show', $vol) }}" class="btn-action" title="View Application">
                            <i class="fas fa-eye"></i>
                        </a>
                        <form method="POST" action="{{ route('admin.volunteers.destroy', $vol) }}" onsubmit="return confirm('Remove this volunteer?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn-action btn-del" title="Remove"><i class="fas fa-trash"></i></button>
                        </form>
                    </div>
                </td>
            </tr>

...
@section('content')
    <div class="page-header">
        <h2 style="font-size:20px;font-weight:700;color:var(--text);">
            <i class="fas fa-images" style="color:#22d3ee;margin-right:10px;"></i>Active Hero Sliders
        </h2>
        <div style="display: flex; gap: 15px; align-items: center;">
            <span style="font-size:13px;color:#64748b;">Total: {{ $sliders->count() }}</span>
            <a href="{{ route('admin.sliders.create') }}" class="btn-edit"
                        ...
                        <tr>
                            <td>
                                <img src="{{ asset('storage/' . $slider->image) }}"
                                    style="width: 100px; height: 60px; object-fit: cover; border-radius: 8px; border: 1px solid var(--border);">
                            </td>
                            <td>
                                <div style="font-weight: 600; color: var(--text);">{{ $slider->title }}</div>
                                <div
                            ...
                            <td>
                                <div style="display: flex; gap: 8px; justify-content: flex-end;">
                                    <form method="POST" action="{{ route('admin.sliders.toggle', $slider) }}">
                                        @csrf
                                        <button type="submit"
                                            class="btn-status {{ $slider->is_active ? 'btn-deactivate' : 'btn-activate' }}">
                                            <i class="fas {{ $slider->is_active ? 'fa-eye-slash' : 'fa-eye' }}"></i>
                                            {{ $slider->is_active ? 'Deactivate' : 'Activate' }}
                                        </button>
                                    </form>
                                    <a href="{{ route('admin.sliders.edit', $slider) }}" class="btn-edit">
